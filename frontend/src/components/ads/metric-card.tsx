...
export function MetricCard({
  ...
  return (
    <div
      className={`group relative overflow-hidden rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition-colors ${th.hover} dark:border-slate-800 dark:bg-slate-900`}
    >
      <div className="relative z-10 mb-2 flex items-center gap-2 text-sm font-medium text-slate-500">
        <div className={`flex h-8 w-8 items-center justify-center rounded-md ${th.iconBg}`}>
          <Icon className="h-4 w-4" />
        </div>
        {label}
      </div>
      <div className="relative z-10 text-3xl font-bold text-slate-900 dark:text-white">{value}</div>
      {subtitle && (
        <div className="relative z-10 mt-1 text-xs font-semibold tracking-wide text-slate-400 uppercase">
          {subtitle}
        </div>
      )}
      {delta.dir !== 'flat' && (
