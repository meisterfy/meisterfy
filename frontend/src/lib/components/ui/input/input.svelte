<script lang="ts">
	...
	let { class: className, value = $bindable(), type = 'text', ...restProps }: HTMLInputAttributes = $props()
...
{#if type === 'password'}
	<div class="relative w-full">
		...
		<button
			type="button"
			tabindex="-1"
			class="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 focus:outline-none dark:hover:text-slate-300"
