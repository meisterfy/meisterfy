...
test('danger variant applies red border class', async () => {
  ...
  await expect.element(screen.getByRole('button', { name: 'Remove' })).toHaveClass('border-red-400/30')
})
...
test('primary variant applies primary background class', async () => {
  ...
  await expect.element(screen.getByRole('button', { name: 'Connect' })).toHaveClass('bg-slate-500/10')
})
