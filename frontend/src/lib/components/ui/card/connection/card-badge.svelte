<script lang="ts">
	...
	const classes = $derived(`inline-flex items-center ${label ? 'px-2 py-1' : 'p-2'} gap-1 border border-white/5 rounded-md text-[10px] font-bold uppercase tracking-wide shrink-0 ${variants[variant] || variants.default} ${className}`)
</script>
