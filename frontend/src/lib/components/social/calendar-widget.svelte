<script lang="ts">
	...
	const calendarCells = $derived.by(() => {
		...
		const byDate: Record<string, PostShape[]> = {}
		...
		for (let d = 1; d <= daysInMonth; d++) {
			...
			cells.push({ date, day: d, posts: byDate[date] ?? [] })
		}
...
<div class="grid grid-cols-7 border-t border-l border-slate-200 dark:border-slate-800">
	{#each calendarCells as cell}
		<div
			class="group/cell relative min-h-27.5 border-r border-b border-slate-200 p-1.5 dark:border-slate-800 {cell.date
...
<div class="mt-4 flex flex-wrap items-center gap-4 text-xs text-slate-500">
	...
	>
	<span class="flex items-center gap-1.5">
		<ProviderIcon provider="instagram" size={12} class="h-3 w-3 text-[#E1306C]" />
		Instagram
	</span>
	<span class="flex items-center gap-1.5">
		<ProviderIcon provider="facebook" size={12} class="h-3 w-3 text-[#1877F2]" />
		Facebook
	</span>
	<span class="flex items-center gap-1.5">
		<ProviderIcon provider="linkedin" size={12} class="h-3 w-3 text-[#0A66C2]" />
		LinkedIn
	</span>
</div>
