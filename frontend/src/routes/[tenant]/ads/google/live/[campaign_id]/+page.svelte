				...
				options: {
					...
					plugins: {
						tooltip: {
							callbacks: {
								label: (ctx) => {
									if (ctx.dataset.label === 'Cost (R$)')
										return `Cost: R$${Number(ctx.raw).toFixed(2)}`
...
>
		...
		>
			{#each data.openAlerts as alert}
				<div class="flex items-start gap-3">
					...
					<div>
						<span
							class="mr-2 text-xs font-bold tracking-wide text-red-700 uppercase dark:text-red-300"
							>{alert.level}</span
						>
	...
	>
		<div
			class="absolute top-0 right-0 -mt-10 -mr-10 h-32 w-32 rounded-full bg-indigo-500/10 blur-3xl"
		></div>
		<div class="relative z-10">
			<div class="mb-2 flex items-center gap-2">
				<span
					class="rounded bg-slate-100 px-2 py-0.5 font-mono text-xs text-slate-500 dark:bg-slate-800"
					>ID: {data.campaign.id}</span
				>
				{#if data.campaign.status === 'ENABLED'}
					<span
						class="inline-flex items-center gap-1 rounded border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[10px] font-bold tracking-wider text-emerald-700 uppercase dark:border-emerald-800/50 dark:bg-emerald-900/30 dark:text-emerald-400"
					>
						<Play class="h-3 w-3" /> Active
					</span>
				{:else}
					<span
						class="inline-flex items-center gap-1 rounded border border-slate-200 bg-slate-100 px-2 py-0.5 text-[10px] font-bold tracking-wider text-slate-600 uppercase dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300"
					>
		...
		<div
			class="relative z-10 flex items-center rounded-lg border border-slate-200 bg-slate-100 p-1 dark:border-slate-700/50 dark:bg-slate-800/50"
		>
			<button
				onclick={() => clearPeriod()}
				disabled={isLoadingPeriod}
				class="rounded-md px-3 py-1.5 text-sm font-medium text-slate-600 transition-colors hover:bg-white hover:shadow-sm disabled:opacity-50 dark:text-slate-300 dark:hover:bg-slate-700"
				>All Time</button
			>
			<button
				onclick={() => setPeriod(7)}
	...
	<div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
		<!-- Impressions -->
		<div
			class="group relative overflow-hidden rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition-colors hover:border-indigo-200 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-indigo-800"
		>
			<div class="absolute top-0 right-0 p-4 opacity-5 transition-opacity group-hover:opacity-10">
				<Activity class="h-16 w-16 text-indigo-500" />
			</div>
			<div class="relative z-10 mb-2 flex items-center gap-2 text-sm font-medium text-slate-500">
				<div
					class="flex h-8 w-8 items-center justify-center rounded-md bg-indigo-50 text-indigo-500 dark:bg-indigo-900/30"
				>
					<Activity class="h-4 w-4" />
				</div>
				Impressions
			</div>
			<div class="relative z-10 text-3xl font-bold text-slate-900 dark:text-white">
				{data.campaign.metrics.impressions}
			</div>
			<div class="relative z-10 mt-1 text-xs font-semibold tracking-wide text-slate-400 uppercase">
				Share: {data.campaign.metrics.searchImpressionShare}
			</div>
				...
				<div class="relative z-10 mt-2 flex items-center gap-1">
					{#if deltas.impressions.dir === 'up'}
						...
						<span class="text-xs font-bold text-emerald-600 dark:text-emerald-400"
							>{deltas.impressions.pct} vs prev. wk.</span
						>
					{:else}
		...
		<!-- Clicks -->
		<div
			class="group relative overflow-hidden rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition-colors hover:border-blue-200 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-blue-800"
		>
			<div class="absolute top-0 right-0 p-4 opacity-5 transition-opacity group-hover:opacity-10">
				<MousePointerClick class="h-16 w-16 text-blue-500" />
			</div>
			<div class="relative z-10 mb-2 flex items-center gap-2 text-sm font-medium text-slate-500">
				<div
					class="flex h-8 w-8 items-center justify-center rounded-md bg-blue-50 text-blue-500 dark:bg-blue-900/30"
				>
			...
			<div class="relative z-10 text-3xl font-bold text-slate-900 dark:text-white">
				{data.campaign.metrics.clicks}
			</div>
			<div class="relative z-10 mt-1 text-xs font-semibold tracking-wide text-slate-400 uppercase">
				CTR: {data.campaign.metrics.ctr}
			</div>
			{#if deltas.clicks.dir !== 'flat'}
				<div class="relative z-10 mt-2 flex items-center gap-1">
					{#if deltas.clicks.dir === 'up'}
						<TrendingUp class="h-3 w-3 text-emerald-500" />
						<span class="text-xs font-bold text-emerald-600 dark:text-emerald-400"
							>{deltas.clicks.pct} vs prev. wk.</span
						>
					{:else}
						<TrendingDown class="h-3 w-3 text-red-400" />
						<span class="text-xs font-bold text-red-500 dark:text-red-400"
		...
		<!-- Cost -->
		<div
			class="group relative overflow-hidden rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition-colors hover:border-emerald-200 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-emerald-800"
		>
			<div class="absolute top-0 right-0 p-4 opacity-5 transition-opacity group-hover:opacity-10">
				<DollarSign class="h-16 w-16 text-emerald-500" />
			</div>
			<div class="relative z-10 mb-2 flex items-center gap-2 text-sm font-medium text-slate-500">
				<div
					class="flex h-8 w-8 items-center justify-center rounded-md bg-emerald-50 text-emerald-500 dark:bg-emerald-900/30"
				>
			...
			{#if deltas.cost.dir !== 'flat'}
				<div class="relative z-10 mt-2 flex items-center gap-1">
					{#if deltas.cost.dir === 'up'}
						<TrendingUp class="h-3 w-3 text-emerald-500" />
						<span class="text-xs font-bold text-emerald-600 dark:text-emerald-400"
							>{deltas.cost.pct} vs prev. wk.</span
						>
					{:else}
						<TrendingDown class="h-3 w-3 text-red-400" />
						<span class="text-xs font-bold text-red-500 dark:text-red-400"
		...
		<!-- Conversions -->
		<div
			class="group relative overflow-hidden rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition-colors hover:border-amber-200 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-amber-800"
		>
			<div class="absolute top-0 right-0 p-4 opacity-5 transition-opacity group-hover:opacity-10">
				<Target class="h-16 w-16 text-amber-500" />
			</div>
			<div class="relative z-10 mb-2 flex items-center gap-2 text-sm font-medium text-slate-500">
				<div
					class="flex h-8 w-8 items-center justify-center rounded-md bg-amber-50 text-amber-500 dark:bg-amber-900/30"
				>
			...
			<div class="relative z-10 text-3xl font-bold text-slate-900 dark:text-white">
				{data.campaign.metrics.conversions}
			</div>
			<div class="relative z-10 mt-1 text-xs font-semibold tracking-wide text-slate-400 uppercase">
				CPA: ${data.campaign.metrics.cpa}
			</div>
			{#if deltas.conversions.dir !== 'flat'}
				<div class="relative z-10 mt-2 flex items-center gap-1">
					{#if deltas.conversions.dir === 'up'}
						<TrendingUp class="h-3 w-3 text-emerald-500" />
						<span class="text-xs font-bold text-emerald-600 dark:text-emerald-400"
							>{deltas.conversions.pct} vs prev. wk.</span
						>
					{:else}
						<TrendingDown class="h-3 w-3 text-red-400" />
						<span class="text-xs font-bold text-red-500 dark:text-red-400"
	...
	{#if data.campaign.history && data.campaign.history.length > 0}
		...
		>
			<h3 class="mb-6 flex items-center gap-2 text-lg font-bold text-slate-900 dark:text-white">
				<Activity class="h-5 w-5 text-indigo-500" /> Performance Timeline
	...
	{#if data.dbHistory.length > 0}
		<div
			class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900"
		>
			<h3 class="mb-1 flex items-center gap-2 text-lg font-bold text-slate-900 dark:text-white">
				<BarChart2 class="h-5 w-5 text-indigo-500" /> Daily Cost + CPA — 30 days
				<span class="ml-auto text-xs font-normal text-slate-400">source: local monitoring</span>
			</h3>
			<p class="mb-5 ml-7 text-xs text-slate-400">CPA plotted only on days with conversions</p>
			<div class="h-[280px] w-full">
					...
					{#each data.campaign.adGroups as group}
						<tr class="transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50">
							...
							<td class="px-6 py-4 text-right font-mono text-slate-600 dark:text-slate-400"
								>{group.metrics.impressions}</td
							>
							<td class="px-6 py-4 text-right font-mono text-slate-600 dark:text-slate-400"
								>{group.metrics.clicks}</td
							>
							<td
								class="px-6 py-4 text-right font-mono font-medium text-emerald-600 dark:text-emerald-400"
								>${group.metrics.cost}</td
							>
							<td
								class="px-6 py-4 text-right font-mono font-bold text-amber-600 dark:text-amber-400"
								>{group.metrics.conversions}</td
							>
						</tr>
					{/each}
