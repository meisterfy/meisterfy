...
<div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
	<div class="border-b border-slate-200 bg-slate-50/50 px-6 py-5 dark:border-slate-800 dark:bg-slate-800/20">
		<h3 class="flex items-center gap-2 text-lg font-bold text-slate-900 dark:text-white">
			<ChartColumnIncreasing class="h-5 w-5 text-indigo-500" /> {m['ads:ad_groups_breakdown']()}
		</h3>
	</div>
			...
			<tbody class="divide-y divide-slate-200 dark:divide-slate-800">
				{#each adGroups as group}
					<tr class="transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50">
						...
						<td class="px-6 py-4 text-right font-mono font-medium text-emerald-600 dark:text-emerald-400">
							{group.metrics.cost}
						</td>
