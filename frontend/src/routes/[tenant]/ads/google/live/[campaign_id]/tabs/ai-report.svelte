	...
	async function generate() {
		...
		try {
			await streamGenerate(
				...
				(chunk) => {
					if (!chunk.done) report += chunk.content
				},
				controller.signal
			)
...
<div class="space-y-4 py-6">
			...
			{:else if availableProviders.length > 1}
				<div class="relative">
					<select
						...
						class="appearance-none rounded-lg border border-slate-200 bg-white px-3 py-2 pr-8 text-sm text-slate-700 focus:ring-2 focus:ring-indigo-500 focus:outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300"
					>
					...
					<ChevronDown
						class="pointer-events-none absolute top-1/2 right-2 h-4 w-4 -translate-y-1/2 text-slate-400"
					/>
				</div>
			{:else}
	...
	{:else if !isGenerating && availableProviders.length > 0}
		<div
			class="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-200 py-16 text-center dark:border-slate-700"
		>
