<script lang="ts">
	...
	function statusBadge(status: string) {
		if (status === 'EXCLUDED') return { label: 'Negative', cls: 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300' }
		if (status === 'ADDED')    return { label: 'Keyword',  cls: 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300' }
		return null
	}
</script>

<div class="rounded-xl border border-white/10 bg-white dark:bg-slate-900 p-4 space-y-4">
	...
	{:else}
		{#if converting.length > 0}
			<section class="space-y-2">
				<div>
					<p class="text-xs font-semibold text-slate-600 dark:text-slate-300">{m['ads:analytics.search_terms_converting']()}</p>
					...
					<table class="w-full text-xs">
						<thead>
							<tr class="border-b border-white/10 text-left text-slate-500">
								<th class="pb-1 pr-3 font-medium">Search Term</th>
								<th class="pb-1 pr-3 font-medium text-right">Clicks</th>
								<th class="pb-1 pr-3 font-medium text-right">Conv.</th>
								<th class="pb-1 pr-3 font-medium text-right">Cost</th>
								<th class="pb-1 pr-3 font-medium text-right">CPA</th>
								<th class="pb-1 pr-3 font-medium text-right">CTR</th>
								<th class="pb-1 font-medium">Status</th>
							</tr>
						</thead>
						<tbody>
							{#each converting as t}
								{@const badge = statusBadge(t.status)}
								<tr class="border-b border-white/5 hover:bg-slate-50 dark:hover:bg-slate-800/50">
									<td class="py-1.5 pr-3 text-slate-700 dark:text-slate-200 max-w-[200px] truncate">{t.term}</td>
									...
									<td class="py-1.5">
										{#if badge}
											<span class="rounded-full px-1.5 py-0.5 text-[10px] font-medium {badge.cls}">{badge.label}</span>
										{/if}
									</td>
								</tr>
							{/each}
		...
		{#if wasted.length > 0}
			<section class="space-y-2">
				<div class="flex items-center justify-between">
					<div>
						<p class="text-xs font-semibold text-slate-600 dark:text-slate-300">{m['ads:analytics.search_terms_wasted']()}</p>
				...
				{#if fallbackCsv !== null}
					<div class="space-y-1">
						...
						<textarea
							readonly
							rows={5}
							class="w-full rounded-md border border-slate-200 bg-slate-50 p-2 text-xs font-mono dark:border-slate-700 dark:bg-slate-800"
