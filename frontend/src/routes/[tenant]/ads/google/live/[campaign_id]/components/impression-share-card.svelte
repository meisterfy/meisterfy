...
{#if stats}
	<div class="rounded-xl border border-white/10 bg-white/5 p-4 space-y-4">
		<h3 class="text-sm font-semibold text-slate-700 dark:text-slate-200">Search Impression Share</h3>
		...
		<div class="flex gap-6 text-sm">
			<div class="flex items-center gap-1.5">
				...
				<span class="font-semibold text-slate-800 dark:text-slate-100">{(stats.wonShare * 100).toFixed(0)}%</span>
			</div>
			<div class="flex items-center gap-1.5">
				...
				<span class="font-semibold text-slate-800 dark:text-slate-100">{(stats.lostBudget * 100).toFixed(0)}%</span>
			</div>
			<div class="flex items-center gap-1.5">
				...
				<span class="font-semibold text-slate-800 dark:text-slate-100">{(stats.lostRank * 100).toFixed(0)}%</span>
			</div>
		</div>

		{#if stats.lostBudget > 0.15}
			<div class="rounded-lg border border-amber-300/40 bg-amber-50 px-3 py-2 text-sm text-amber-800 dark:bg-amber-900/20 dark:text-amber-300">
				Budget is limiting reach — consider increasing daily budget
			</div>
		{/if}
		{#if stats.lostRank > 0.15}
			<div class="rounded-lg border border-red-300/40 bg-red-50 px-3 py-2 text-sm text-red-800 dark:bg-red-900/20 dark:text-red-300">
