...
let pendingPerms = $state<Set<string>>(new Set())
let pendingName = $state('')
let hasPendingChanges = $derived(
selectedRole != null &&
!isSystemRole(selectedRole) &&
(JSON.stringify([...pendingPerms].sort()) !== JSON.stringify([...(selectedRole.permissions ?? [])].sort()) || pendingName !== selectedRole.name)
)
...
{ key: 'content', label: () => m['settings:roles_perm_group_content'](), match: (p) => p.includes('post') || p.includes('content') },
{ key: 'advertising', label: () => m['settings:roles_perm_group_advertising'](), match: (p) => p.includes('campaign') || p.includes('report') || p.includes('analytics') },
{ key: 'users', label: () => m['settings:roles_perm_group_users'](), match: (p) => p.includes('user') || p.includes('role') || p.includes('tenant') },
{ key: 'integrations', label: () => m['settings:roles_perm_group_integrations'](), match: (p) => p.includes('integration') || p.includes('automation') }
]
...
if (perms.length > 0) {
groups.push({ key: g.key, label: g.label(), perms: perms.map((p) => ({ name: p, has: permSet.has(p) })) })
}
}

const others = all.filter((p) => !covered.has(p))
if (others.length > 0) {
groups.push({ key: 'other', label: 'Other', perms: others.map((p) => ({ name: p, has: permSet.has(p) })) })
}
...
const matchFn = g ? g.match : (p: string) => {
// for "other" group
...
updateFn(new Set(allPermissions.map((p) => p.name)))
}
}
...
isSaving = true
try {
await updateRole(selectedRole.id, { name: pendingName.trim(), permissions: [...pendingPerms] })
selectedRole.permissions = [...pendingPerms]
selectedRole.name = pendingName.trim()
roles = roles.map((r) => r.id === selectedRole!.id ? { ...r, permissions: [...pendingPerms], name: pendingName.trim() } : r)
...
{/snippet}
{#if canCreate}
<Button onclick={() => (showCreateModal = true)} class="flex items-center gap-2 h-9 px-3 text-sm">
<Plus class="h-4 w-4" />
...
<!-- roles list -->
<div>
<div class="flex items-end justify-between font-semibold text-primary text-xs mb-2">
<span>Name</span>
...
<button
class="flex w-full items-center justify-between border-b last:border-b-0 border-border 
...
>
<span class="truncate flex flex-row items-center justify-start gap-1.5">
<h3 class="text-base font-semibold">
{roleLabel(role.name)}
</h3>
<span class="rounded px-1.5 py-0.5 text-[10px] uppercase font-bold tracking-wide {isSystemRole(role) ? 'bg-muted text-muted-foreground' : 'bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-400'}">
{isSystemRole(role) ? m['settings:roles_system_badge']() : m['settings:roles_custom_badge']()}
</span>
</span>
<div class="flex shrink-0 items-center gap-1.5 ml-2">
<span class="rounded px-1.5 py-0.5 text-xs font-medium tabular-nums {isSystemRole(role) ? 'bg-muted text-muted-foreground' : 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400'}">
...
{@const groups = groupPermissions(isSystem ? new Set(currentRole.permissions ?? []) : pendingPerms)}
...
<Input bind:value={pendingName} placeholder="Role Name" class="max-w-[300px] h-9 text-base font-semibold" />
{:else}
<h2 class="text-base lg:text-xl font-semibold">
...
<Square class="h-3.5 w-3.5 mr-1" /> Deselect All
{:else}
<CheckSquare class="h-3.5 w-3.5 mr-1" /> Select All
{/if}
</Button>
{/if}
...
<Button
onclick={() => { deleteTarget = currentRole; showDeleteDialog = true }}
variant="red"
class="h-9"
>
...
<div>
<div class="flex items-center justify-between border-b border-border pb-2 mb-2.5">
...
class="text-[10px] font-semibold text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300 transition-colors uppercase tracking-wider px-1 py-0.5 rounded hover:bg-indigo-50 dark:hover:bg-indigo-900/30"
>
{allChecked ? 'None' : 'All'}
</button>
{/if}
...
<label class="flex cursor-pointer items-center gap-2.5 rounded-md py-1 text-sm transition-colors {!isSystem && canUpdate ? 'hover:bg-muted/50' : ''}">
<input
type="checkbox"
checked={perm.has}
...
{:else}
<div class="border-border flex h-48 items-center justify-center rounded-xl border bg-bg-light">
...
<div>
<label for="new-role-name" class="text-muted-foreground mb-1 block text-xs font-semibold uppercase tracking-wide">
{m['settings:roles_create_field_name']()}
</label>
<Input id="new-role-name" bind:value={newRoleName} placeholder="e.g. Editor" class="max-w-[400px]" />
</div>

<div>
<div class="flex items-center justify-between mb-4">
<p class="text-muted-foreground text-xs font-semibold uppercase tracking-wide">Permissions</p>
<Button
...
<div class="flex flex-col gap-1.5">
{#each group.perms as perm (perm.name)}
<label class="flex cursor-pointer items-center gap-2.5 rounded py-1 text-sm hover:bg-muted/50 transition-colors">
<input
type="checkbox"
checked={perm.has}
...
/>
<span class="{perm.has ? '' : 'text-muted-foreground'}">{permLabel(perm.name)}</span>
</label>
{/each}
</div>
</div>
...
<Button onclick={handleCreate} disabled={isCreating || !newRoleName.trim()} class="h-10 px-6 text-sm">
...
{#if toast}
<div
class="fixed bottom-6 right-6 z-50 rounded-lg px-4 py-3 text-sm font-medium shadow-lg {toast.error
