<script lang="ts">
	...
	async function save(e: SubmitEvent) {
		e.preventDefault()
		if (!name.trim()) {
			errorMsg = m['settings:error_required_name']()
			return
		}
		errorMsg = null
		isSaving = true
	...
	<div class="flex flex-col gap-6 lg:flex-row lg:gap-8">
		<div class="lg:w-1/3">
			<h2 class="text-base font-semibold text-slate-900 dark:text-white">
				{m['settings:brand_identity_title']()}
			</h2>
			<p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
				{m['settings:brand_identity_desc']()}
			</p>
		</div>
				...
				<div class="flex flex-col gap-5">
					<div class="grid gap-5 sm:grid-cols-2">
						...
						<div>
							...
							<input
								...
								type="text"
								bind:value={niche}
								placeholder={m['settings:placeholder_niche']()}
								class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 focus:ring-2 focus:ring-indigo-500 focus:outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-white"
							/>
						</div>
					</div>
					...
					<div class="grid gap-5 sm:grid-cols-2">
						...
						<div>
							...
							<input
								...
								type="text"
								bind:value={location}
								placeholder={m['settings:placeholder_location']()}
								class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 focus:ring-2 focus:ring-indigo-500 focus:outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-white"
							/>
						</div>
					</div>
					...
					<div>
						...
						<input
							...
							placeholder={m['settings:placeholder_persona']()}
							class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 focus:ring-2 focus:ring-indigo-500 focus:outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-white"
						/>
					</div>
					...
					<div>
						...
						<input
							...
							type="text"
							bind:value={tone}
							placeholder={m['settings:placeholder_tone']()}
							class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 focus:ring-2 focus:ring-indigo-500 focus:outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-white"
						/>
					</div>
					...
					<div>
						...
						<input
							...
							placeholder={m['settings:placeholder_hashtags']()}
