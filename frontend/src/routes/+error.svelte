...
<div class="flex flex-1 items-center justify-center p-8">
	<div class="w-full max-w-md space-y-6">
		...
		<div class="flex items-end gap-3">
			<span
				class="text-8xl leading-none font-black tracking-tighter tabular-nums
				{is5xx ? 'text-red-500 dark:text-red-400' : 'text-amber-500 dark:text-amber-400'}"
			>
				{status}
			</span>
			<div
				class="border-l-2 pb-2 pl-3
				{is5xx ? 'border-red-200 dark:border-red-800' : 'border-amber-200 dark:border-amber-800'}"
			>
				<p class="text-lg leading-snug font-bold text-slate-800 dark:text-slate-100">
					{label.title}
				</p>
		...
		<div class="flex gap-2 pt-1">
			...
			<button
				...
				class="flex-1 rounded-lg bg-slate-100 px-4 py-2.5 text-sm
