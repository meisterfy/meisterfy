...
function emptyTranslations(): Record<string, LegalBlock[]> {
  return { en: [{ title: '', content: '' }], 'pt-BR': [{ title: '', content: '' }] }
}
...
export function LegalRoute() {
  ...
  const [editTranslations, setEditTranslations] = useState<Record<string, LegalBlock[]>>(
    emptyTranslations(),
  )
      ...
      <aside className='flex w-64 shrink-0 flex-col border-r border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950'>
        ...
        <div className='flex-1 overflow-y-auto'>
          ...
          {isNew && (
            ...
            >
              <p className='text-sm font-medium text-primary'>{t('legal_new_version')}</p>
            </button>
          )}
          ...
          {versions.map((v) => (
            <button
              ...
              className={`w-full border-b border-slate-200 px-4 py-3 text-left transition-colors hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-900 ${
                selectedId === v.id && !isNew ? 'bg-muted dark:bg-slate-800' : ''
              }`}
            >
              ...
              <p className='mt-0.5 text-xs text-muted-foreground'>
                {t('legal_effective_label', { date: formatDate(v.effective_at) })}
              </p>
            </button>
          ))}
        </div>
