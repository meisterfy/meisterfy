  ...
  const load = useCallback(
    async (params: {
      ...
      const chatSystemPrompt = detail
        ? buildChatSystemPrompt(
            brand,
            buildCampaignData(detail, searchTerms, keywords, qualityScores),
          )
        : ''
      ...
      <Tabs defaultValue="live">
        <TabsList className="flex items-center justify-start gap-2 border-b border-white/10 py-4 lg:gap-4">
          ...
          <TabTrigger value="history">
            {t('headings.campaign_history')}
          </TabTrigger>
          <TabTrigger value="search">
            {t('headings.search_intelligence')}
          </TabTrigger>
          <TabTrigger value="ai">
