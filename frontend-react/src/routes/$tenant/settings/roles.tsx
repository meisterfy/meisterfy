...
import { isSystemRole, togglePerm, toggleGroup, selectAllToggle } from '@/features/roles/roles-permissions'
...
export function RolesRoute() {
  ...
  const canCreate = useAuth((s) => s.user?.permissions?.includes('create:role') ?? false)
  const canUpdate = useAuth((s) => s.user?.permissions?.includes('update:role') ?? false)
  const canDelete = useAuth((s) => s.user?.permissions?.includes('delete:role') ?? false)
  ...
  return (
    <>
        ...
        <div className='grid gap-4 lg:grid-cols-[280px_1fr] xl:grid-cols-[320px_1fr]'>
          ...
          {selectedRole ? (
            <RoleDetailCard
              ...
              isSaving={isSaving}
              onSelectAll={() => setPendingPerms((s) => selectAllToggle(s, allPermissions))}
              ...
              onToggleGroup={(k) => setPendingPerms((s) => toggleGroup(allPermissions, s, k))}
            />
          ) : (
            <div className='border-border bg-bg-light flex h-48 items-center justify-center rounded-xl border'>
              <p className='text-muted-foreground text-sm'>{t('roles_no_selection')}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <CreateRoleDrawer
        ...
        onSelectAll={() => setNewRolePerms((s) => selectAllToggle(s, allPermissions))}
        onTogglePerm={(n) => setNewRolePerms((s) => togglePerm(s, n))}
        onToggleGroup={(k) => setNewRolePerms((s) => toggleGroup(allPermissions, s, k))}
