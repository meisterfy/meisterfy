...
export function GoogleAdsRoute() {
  ...
  async function saveAdsConfig() {
    ...
    try {
      ...
      setAdsError(err instanceof Error ? err.message : tGlobals('error_generic'))
    } finally {
  ...
  async function savePrompts() {
    ...
    try {
      ...
      setPromptsError(err instanceof Error ? err.message : tGlobals('error_generic'))
    } finally {
  ...
  function CardHeader({ title, subtitle }: { title: string; subtitle: string }) {
    return (
      <div className='flex flex-col gap-1'>
        <p className='text-text text-sm font-medium lg:text-lg xl:text-xl'>{title}</p>
          ...
          {statusLoading ? (
            ...
            <>
              ...
              <CardAside
                ...
                footer={<SaveButton isSaving={isSavingAds} saved={savedAds} onClick={saveAdsConfig} />}
              >
                {llmConnected && (
                ...
                header={
                  <div className='flex items-end justify-between gap-4'>
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) => setAdjustmentsEnabled(checked)}
                    />
                  </div>
                }
              >
                ...
                header={
                  <div className='flex items-end justify-between gap-4'>
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) => setSuggestionsEnabled(checked)}
                    />
                  </div>
                }
                footer={<SaveButton isSaving={isSavingAds} saved={savedAds} onClick={saveAdsConfig} />}
          ...
          <div className='divide-y divide-slate-100 dark:divide-slate-800'>
            {PROMPT_PARAMS.map((param) => (
              <div key={param.key} className='flex items-start gap-3 py-3'>
                ...
                <div>
                  <p className='text-sm text-slate-700 dark:text-slate-300'>{param.description}</p>
