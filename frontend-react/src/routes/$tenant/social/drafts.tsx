...
import { getConnectorResources, type ConnectorResource } from '@/lib/api/connector-resources'
...
export function DraftsRoute() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (!active) return
      setDrafts(
        all.filter((p) => p.status === 'draft' || p.status === 'approved').map(normalizePost),
      )
  ...
  const [selectedForSchedule, setSelectedForSchedule] = useState<PostShape | null>(null)

  // ── Publish ────────────────────────────────────────────────────────────────
  const [showPublish, setShowPublish] = useState(false)
  const [selectedForPublish, setSelectedForPublish] = useState<PostShape | null>(null)
  ...
  async function toggleApprove(post: PostShape) {
    ...
    try {
      ...
      setDrafts((ds) => ds.map((d) => (d.id === post.id ? { ...d, status: newStatus } : d)))
    } finally {
      setApprovingId(null)
    }
  }
      ...
      <div className="mx-auto w-full max-w-5xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            <div className="mb-0.5 flex items-center gap-2">
              ...
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">Drafts</h2>
              <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-bold text-slate-500 dark:bg-slate-800">
                {drafts.length}
              </span>
            </div>
            <p className="text-sm text-slate-500 dark:text-slate-400">{t('drafts_subtitle')}</p>
          </div>
        ...
        {drafts.length === 0 ? (
          <div className="rounded-xl border-2 border-dashed border-slate-300 p-16 text-center dark:border-slate-700">
            ...
            <p className="mb-3 text-sm text-slate-500 dark:text-slate-400">{t('no_drafts_yet')}</p>
            <button
              onClick={() => setShowCreate(true)}
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="mb-2 flex flex-wrap items-center gap-2">
                    ...
                    <span className="truncate font-mono text-xs text-slate-400">{post.id}</span>
                  </div>
                  <p className="mb-1 font-semibold text-slate-900 dark:text-white">{post.title}</p>
