...
export function PostEditorRoute() {
  ...
  function buildSystemPrompt(): string {
    ...
    const lang =
      b.language === 'pt_BR' ? 'Brazilian Portuguese' : b.language === 'es_ES' ? 'Spanish' : 'English'
          ...
          <div className="fixed top-0 right-0 z-30 flex h-full w-full max-w-md flex-col border-l border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-900">
            ...
            {brand && (
              <div className="mx-5 mt-4 rounded-lg border border-indigo-100 bg-indigo-50 px-3 py-2 text-xs text-indigo-700 dark:border-indigo-800 dark:bg-indigo-900/20 dark:text-indigo-300">
                {t('ai_brand_context_label')} <span className="font-semibold">{brand.name}</span>
      ...
      <div className="mx-auto grid max-w-5xl grid-cols-1 gap-8 p-6 lg:grid-cols-3">
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          <div>
            <label
              ...
              className="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300"
            >
              Hashtags <span className="font-normal text-slate-400">{t('hashtags_hint')}</span>
            </label>
            <input
              id="post-hashtags"
              type="text"
        ...
        <div className="space-y-6">
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/50">
            <h3 className="mb-3 text-sm font-bold text-slate-900 dark:text-white">Media</h3>
            ...
            {post.media_files?.length > 0 ? (
              ...
              <div className="mb-4 flex aspect-video items-center justify-center rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 text-slate-400 dark:border-slate-600 dark:bg-slate-800/50">
                <span className="text-xs font-medium">{t('no_media_attached')}</span>
              </div>
            )}
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-700 dark:text-slate-300">
                    <span className="font-medium">{post.workflow.strategy?.framework}:</span>{' '}
          ...
          {publishResults.length > 0 && (
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/50">
              <h3 className="mb-3 text-sm font-bold text-slate-900 dark:text-white">Publicações</h3>
              <div className="space-y-2">
                {publishResults.map((result) => {
                  const cfg = PLATFORM_CONFIG[result.platform as keyof typeof PLATFORM_CONFIG]
                  const color = cfg?.color ?? '#64748b'
                  return (
                    <div key={result.id} className="flex items-start justify-between gap-2 text-sm">
