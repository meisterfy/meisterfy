<script lang="ts">
	...
	import { File, Check } from 'lucide-svelte'

	import type { Post, PostStatus } from '$lib/api/posts'

	type PostItem = Post & { filename: string; media_files: string[] }
	...
	let drafts = $derived(posts.filter((p: PostItem) => p.status === 'draft'))
	let approved = $derived(posts.filter((p: PostItem) => p.status === 'approved'))
</script>
...
<div class="flex h-full w-max min-w-full gap-6">
	...
	<div class="flex h-full w-80 flex-col rounded-xl bg-slate-100/50 dark:bg-slate-900/50">
		<div class="flex items-center gap-2 border-b border-slate-200/50 p-4 dark:border-slate-800/50">
			<File class="h-4 w-4 text-amber-600" />
