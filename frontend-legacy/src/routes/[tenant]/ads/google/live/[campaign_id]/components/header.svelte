<script lang="ts">
	...
	import { ArrowLeft, Download, Target, Play, Pause, RefreshCw, LoaderCircle } from 'lucide-svelte'
	import { m } from '$lib/paraglide/messages'
	import { formatStrategy } from '$lib/utils/ads'
	import type { LiveCampaignDetail } from '$lib/api/campaigns'

	let { detail, tenant, campaignId, actions } = $props<{
		detail: LiveCampaignDetail | null
		tenant: string
		campaignId: string
		actions: ReturnType<typeof import('$lib/runes/campaign-actions.svelte').createCampaignActions>
	}>()
</script>

<div
	class="flex flex-col items-end justify-between gap-2 border-b border-white/10 pb-4 lg:flex-row"
>
	<div class="flex flex-col items-start justify-start gap-2 lg:gap-4">
		<a
			href={resolve(`/${tenant}/ads/google`)}
			class="text-slate-500 hover:text-slate-900 dark:hover:text-slate-300"
		>
			<ArrowLeft class="h-4 w-4" />
		</a>
		<h1 class="text-3xl font-bold tracking-tight text-slate-900 lg:text-4xl dark:text-white">
		...
		<div class="flex items-start justify-start gap-2 lg:gap-4">
			<span
				...
				>ID: {detail?.campaign.id ?? campaignId}</span
			>
			{#if detail}
				<span
					class="inline-flex items-center gap-1 rounded border {detail.campaign.status === 'ENABLED'
						...
						: 'border-slate-200 bg-slate-100 text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300'} px-2 py-0.5 text-[10px] font-bold tracking-wider uppercase"
				>
