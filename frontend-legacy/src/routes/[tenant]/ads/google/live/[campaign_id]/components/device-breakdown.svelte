<script lang="ts">
	...
	const deviceLabel = (d: string) =>
		d === 'DESKTOP'
			? m['ads:analytics.device_desktop']()
			: d === 'MOBILE'
				? m['ads:analytics.device_mobile']()
				: m['ads:analytics.device_tablet']()
	...
	const donutConfig = $derived(() => ({
		...
		options: {
			...
			plugins: {
				...
				tooltip: {
					callbacks: {
						label: (ctx: { parsed: number }) => ` R$${ctx.parsed.toFixed(2)}`
					}
				}
			}
		}
	...
	const cpaDevices = $derived(
		devices
			.filter((d: DeviceRow) => d.conversions > 0)
			.sort((a: DeviceRow, b: DeviceRow) => a.cpa - b.cpa)
	)

	let canvas = $state<HTMLCanvasElement | undefined>()
	let instance: Chart<'doughnut'> | null = null
...
>
	...
	{#if devices.length === 0}
		<p class="py-8 text-center text-sm text-slate-400">{m['ads:analytics.device_empty']()}</p>
	{:else}
							...
							<div class="space-y-1">
								...
								<div class="h-2 rounded-full bg-slate-100 dark:bg-slate-800">
									<div
										...
										style="width: {maxCpa > 0
											? ((d.cpa / maxCpa) * 100).toFixed(1)
											: 0}%; background: {DEVICE_COLORS[d.device] ?? '#94a3b8'}"
									></div>
								</div>
							</div>
						{/each}
