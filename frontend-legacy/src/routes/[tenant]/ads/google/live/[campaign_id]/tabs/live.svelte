<script lang="ts">
	...
	let { detail, isLoadingPeriod, onSetPeriod, onClearPeriod, devices, hourly, impressionShare } =
		$props<{
...
<div class="relative space-y-6 py-6 {isLoadingPeriod ? 'pointer-events-none opacity-50' : ''}">
	...
	{#await detail}
		<div class="grid animate-pulse grid-cols-2 gap-4 lg:grid-cols-3 xl:grid-cols-6">
	...
	{:then d}
		{#if d}
			...
			{@const cvrValue =
				d.wow.cur.clicks > 0
					? ((d.wow.cur.conversions / d.wow.cur.clicks) * 100).toFixed(2) + '%'
					: '—'}
			...
			<div class="grid grid-cols-2 gap-4 lg:grid-cols-3 xl:grid-cols-6">
				<MetricCard
					...
					value={d.campaign.metrics.impressions}
					subtitle={`${m['ads:share']()}: ${d.campaign.metrics.searchImpressionShare}`}
				...
				<MetricCard
					...
					theme="blue"
					label={m['ads:clicks']()}
					value={d.campaign.metrics.clicks}
					subtitle={`${m['ads:ctr']()}: ${d.campaign.metrics.ctr}`}
					delta={deltas.clicks}
				/>
				<MetricCard
					icon={DollarSign}
					theme="emerald"
					label={m['ads:total_cost']()}
					value={d.campaign.metrics.cost}
					delta={deltas.cost}
				/>
				<MetricCard
					icon={Target}
					theme="amber"
					label={m['ads:conversions']()}
					value={d.campaign.metrics.conversions}
					subtitle={`${m['ads:cpa']()}: ${d.campaign.metrics.cpa}`}
			...
			{#if d.campaign.history.length > 0}
				<PerformanceChart
					config={createPerformanceTimelineConfig(d.campaign.history)}
					title={m['ads:graph.performance_timeline']()}
					source={m['ads:graph.source']({ source: 'Google Ads API' })}
					icon={Activity}
				/>
			{/if}
